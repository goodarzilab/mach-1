...
parser.add_argument(
    "-t", "--trainer_config", type=str, help="Path to trainer config file"
)
parser.add_argument("-m", "--model_config", type=str, help="Path to model config file")
parser.add_argument(
    "-p", "--num_threads", type=int, help="Number of threads", default=16
)
...
if config.from_pretrained:
    model = StripedHyenaModelForCausalLM.from_pretrained(
        config.from_pretrained, config=my_model_config
    )
...
training_args = TrainingArguments(
    ...
    if config.eval_accumulation_steps
    else 128,
    logging_strategy="steps",
    logging_first_step=True,
    logging_steps=config.logging_steps
    if config.logging_steps
    else int(steps_per_train_epoch * 0.025),
...
class WeightDecayedTrainer(Trainer):
    def get_decay_parameter_names(self, model) -> List[str]:
        ...
        decay_parameters = [
            ...
            if not any(
                no_decay_layer_name in name
                for no_decay_layer_name in no_decay_layer_names
            )
        ]
...
class TotalFlopsCallback(TrainerCallback):
    def on_evaluate(self, args, state, control, **kwargs):
        ...
        wandb.log(
            {"eval_ppl": eval_ppl, "total_flos": state.total_flos}
        )  # , step=state.global_step)


MyCallbacks = []
if config.compute_flops:
    MyCallbacks += [TotalFlopsCallback()]
